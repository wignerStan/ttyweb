...
type Options struct {
	...
	MaxConnection       int      `hcl:"max_connection" flagName:"max-connection" flagDescribe:"Maximum connection to gotty (0 for unlimited)" default:"20"`
	...
	CORSAllowedOrigins  []string `hcl:"cors_allowed_origins" flagName:"cors-origins" flagDescribe:"Comma-separated list of allowed CORS origins (default: same-origin only)" default:""`
	...
	DBPath              string   `hcl:"db_path" flagName:"db" flagDescribe:"SQLite database path (default: ~/.local/share/ttyweb/ttyweb.db)" default:""`

	TitleVariables map[string]any
}

// Validate checks the options for conflicting settings.
