...
export default function useShakeDetect(onShake: () => void, opts: ShakeOptions = {}) {
  ...
  const requestPermission = useCallback(async () => {
    ...
    if (typeof DME !== 'undefined' && typeof (DME as unknown as { requestPermission?: () => Promise<string> }).requestPermission === 'function') {
      try {
        const result = await (DME as unknown as { requestPermission: () => Promise<string> }).requestPermission();
