...
export function MobileTerminal({ session, pane, fontSize, onFontSizeChange, voiceRef, taskHistoryPaneKey, onStatusChange }: MobileTerminalProps) {
  ...
  useEffect(() => {
    if (termRef.current && fitRef.current) {
      ...
      setTimeout(() => {
        ...
        fitRef.current.fit();
        if (wsRef.current?.readyState === WebSocket.OPEN) {
          wsRef.current.send('3' + JSON.stringify({ columns: termRef.current.cols, rows: termRef.current.rows }));
        }
      }, 300);
    }
  }, [fontSize]);
  ...
  useEffect(() => {
    ...
    const connect = () => {
      ...
      ws.onopen = () => {
        ...
        if (termRef.current) {
          ws.send('3' + JSON.stringify({ columns: termRef.current.cols, rows: termRef.current.rows }));
        }
    ...
    const shouldSuppressBurstIOS = (data: string, now: number): boolean => {
      if (data === ' ') {
        ...
        if (spaceTimestamps.length >= SPACE_BURST_COUNT) {
          telemetryLog('suppressed', { data: JSON.stringify(data), reason: 'space-burst', count: spaceTimestamps.length });
          emitter.emit('mobile-suppress', { reason: 'space-burst', data: JSON.stringify(data), count: spaceTimestamps.length });
      ...
      if (data === '\r' || data === '\n') {
        ...
        if (enterTimestamps.length >= ENTER_BURST_COUNT) {
          telemetryLog('suppressed', { data: JSON.stringify(data), reason: 'enter-burst', count: enterTimestamps.length });
          emitter.emit('mobile-suppress', { reason: 'enter-burst', data: JSON.stringify(data), count: enterTimestamps.length });
      ...
      if (elapsed < BURST_SUPPRESSION_WINDOW_MS) {
        telemetryLog('suppressed', { data: JSON.stringify(data), transitionType: transition.type, elapsed });
        emitter.emit('mobile-suppress', { reason: 'post-transition', data: JSON.stringify(data), transitionType: transition.type, elapsed });
        return true;
      }
      return false;
    };
