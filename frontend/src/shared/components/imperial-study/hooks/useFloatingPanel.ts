...
export function useFloatingPanel(options?: UseFloatingPanelOptions) {
  ...
  const [opacity, setOpacity] = useState(() => loadFromStorage(`${storageKey}-opacity`, 0.15))

  // Persist state
  useEffect(() => {
    saveToStorage(`${storageKey}-collapsed`, collapsed)
  }, [collapsed, storageKey])
  useEffect(() => {
    saveToStorage(`${storageKey}-pos`, position)
  }, [position, storageKey])
  useEffect(() => {
    saveToStorage(`${storageKey}-size`, size)
  }, [size, storageKey])
  useEffect(() => {
    saveToStorage(`${storageKey}-opacity`, opacity)
  }, [opacity, storageKey])
