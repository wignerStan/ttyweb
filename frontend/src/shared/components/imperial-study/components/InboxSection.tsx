...
import { INBOX_KIND_CONFIG, INBOX_KIND_DEFAULT_COLOR, INBOX_KIND_DEFAULT_ICON } from '../constants'
...
function InboxCard({ item, onClick }: InboxCardProps) {
  ...
  return (
    ...
    >
      ...
      {item.body && <div className="is-inbox-body-preview">{item.body}</div>}
    ...
    <div className="is-section">
      ...
      >
        {items.length === 0 ? (
          ...
          items.map((item) => <InboxCard key={item.id} item={item} onClick={onItemClick} />)
        )}
      </div>
    </div>
  )
