...
describe('NotificationProvider (hook)', () => {
  ...
  it('dismisses a notification', () => {
    ...
    act(() => {
      result.current.notify({ type: 'info', title: 'Test', message: 'Hello', paneKey: '' })
    })
  ...
  it('handles multiple notifications', () => {
    const { result } = renderHook(() => useNotification(), { wrapper: createWrapper() })
    act(() => {
      result.current.notify({ type: 'info', title: 'A', message: '', paneKey: '' })
      result.current.notify({ type: 'ai_completion', title: 'B', message: '', paneKey: '' })
      result.current.notify({ type: 'ai_approval', title: 'C', message: '', paneKey: '' })
    })
